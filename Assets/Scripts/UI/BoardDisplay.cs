...
namespace Chessed
{
    public class BoardDisplay : MonoBehaviour
    {
        ...
        [SerializeField] private ArrowRenderer bestMoveArrow;
        ...
        private void UpdateBestMoveArrow()
        {
            MoveSquares bestMoveSquares = stockfish.BestMoveSquares;
            if (bestMoveSquares == null || !manager.IsCurrentPlayer)
            {
                bestMoveArrow.gameObject.SetActive(false);
                return;
            }
            
            int fromIndex = Board.SquareToIndex(bestMoveSquares.From);
            int toIndex = Board.SquareToIndex(bestMoveSquares.To);
            
            bestMoveArrow.from = squareDisplays[fromIndex].RectTransform;
            bestMoveArrow.to = squareDisplays[toIndex].RectTransform;
            
            bestMoveArrow.gameObject.SetActive(true);
        }
